...
//console.info('inside the loader')
   ...
   NREUM.info = {
       beacon: "bam.nr-data.net",
       errorBeacon: "bam.nr-data.net",
       licenseKey: "d2213c24e1",
       applicationID: "80378645",
       sa: 1
   }


//We now need to now identify the page using the New Relic SetPageViewName method

try  {
  //We need to trigger the load event again
  dispatchEvent(new Event('load'));
} catch (err) {
  console.log("NEW RELIC JS AGENT ERROR Attempt 1: " + err.message);

  try  {
    //We need to trigger the load event again for IE
    var load_event = document.createEvent("Event");
    load_event.initEvent("load", false, false);
    window.dispatchEvent(load_event);
  } catch (err) {
    console.log("NEW RELIC JS AGENT ERROR Attempt 2: " + err.message);
  }
}

if (inIframe() == true){

  //Dynamics 2016 update caused the return of the formSelector.getCurrentItem to return null when there is only one form item
  var count = Xrm.Page.ui.formSelector.items.getLength();
  if (count > 1){

    var formLabel = Xrm.Page.ui.formSelector.getCurrentItem().getLabel();
    if (formLabel != null){
      var stringPage = formLabel.replace (/\//g, "_");
      var subStringPage = stringPage.replace (/ /g, "_");
      //alert(subStringPage);
      newrelic.setPageViewName(subStringPage, '');
    }

  }else{
    var formLabel = Xrm.Page.ui.formSelector.getLabel();
    if (formLabel != null){
      var stringPage = formLabel.replace (/\//g, "_");
      var subStringPage = stringPage.replace (/ /g, "_");
      //alert(subStringPage);
