...
function FormOnload(executionContext) {
    ...
    ArupLead.ApplyLeadBusinessRules( executionContext );
}
...
ArupLead = (
    function () {
        ...
        // This is a Promise. 
        // GetUserDetails can be used as below as many times as required, but will only ever make one call to the server.
        // Usage: -
        // GetUserDetails.then( (result) => DoSomethingWith(result.arup_useownaccountingcentreforopportunities)) ....
        var GetUserDetails = Xrm.WebApi.retrieveRecord("systemuser", Xrm.Utility.getGlobalContext().userSettings.userId, "?$select=arup_useownaccountingcentreforopportunities,_ccrm_accountingcentreid_value,_ccrm_arupcompanyid_value,_ccrm_arupofficeid_value,_ccrm_arupregionid_value");
        // Make devs aware of any issues via the console.
        GetUserDetails.catch(function (error) {console.log("Failure in GetUserDetails: " + error.message)});

        var ArupInterfaceSettings = Xrm.WebApi.retrieveMultipleRecords("ccrm_arupinterfacesetting", "?$select=ccrm_setting&$filter=ccrm_name eq ('Arup.QualifyLead.UserId')");
        ...
        /**
         * Set submit mode on given attribute if present.
         * @param {any} formContext 
         * @param {any} attribute
         * @param {any} submitMode
         */
