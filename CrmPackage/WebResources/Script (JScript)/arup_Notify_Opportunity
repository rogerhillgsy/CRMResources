...
Notify._jQuery = window.jQuery ? window.$ : parent.$;
...
Notify._prefix = "arup_"; // Change this if you have your own solution prefix (as long as the file structure's the same)
...
Notify._crmContainerId = "mainContent"; //For Dynamic Apps Support
...
Notify.addOpp = function(message, level, uniqueId, buttons, durationSeconds, bgColor, textColor) {
    ...
    var $elem = Notify.$("<div>", {
        id: "notifyNotification_" + uniqueId,
        class: "notify-notification"
    }).css("color", textColor).css("background-color", bgColor).hide().prependTo(Notify.$("#notify"));
    var $table = Notify.$("<table>", {
        cellpadding: "0",
        cellspacing: "0"
    }).css("width", "100%").appendTo($elem);
    var $tr = Notify.$("<tr>").appendTo($table);
    if (level && ["INFO", "WARNING", "ERROR", "SUCCESS", "QUESTION", "LOADING", "SEARCH"].indexOf(level) !== -1) {
        var $imgTd = Notify.$("<td>", {
            valign: "top"
        }).css("width", "22px").appendTo($tr);
    ...
    var $textTd = Notify.$("<td>", {
        class: "notify-text"
    }).appendTo($tr);
    var $close = Notify.$("<a>", {
        title: "Dismiss",
        class: "notify-close"
    }).css("color", textColor).click(function() {
    ...
    if (buttons && buttons.length > 0) {
        for (var i = 0; i < buttons.length; i++) {
            var b = buttons[i];
            var $button = Notify.$("<a>", {
                class: b.type == "link" ? "notify-link" : "notify-button"
            });
...
Notify._initialise = function() {
    if (!Notify._initialised) {
        ...
        var $notify = Notify.$("<div>", {
            id: "notifyWrapper"
        });
        $notify.append(Notify.$("<div>", {
            id: "notify",
            class: "notify",
            size: "3",
            maxheight: "51"
        ...
        if ($header.length > 0) {
            ...
            Notify.$("<link/>", {
                rel: "stylesheet",
                href: baseUrl + "/WebResources/" + Notify._prefix + "/css/notify.css"
            }).appendTo($notify);

            Notify._initialised = true;
        } else ($header.length == 0) {
        
             $header = $("#" + Notify._crmContainerId);
             if ($header.length > 0) {
                    $header.prepend($notify);
             }
